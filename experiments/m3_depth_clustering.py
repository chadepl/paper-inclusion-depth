...
if __name__ == "__main__":
    ...
    from backend.src.vis_utils import plot_contour_spaghetti, plot_grid_masks, plot_band_checking_procedure, \
    ...
    # - plot bands sample
    subset_band_sample = [depth_data["subset_data"][i]["band_components"]["band"] for i in
                          np.random.randint(0, len(depth_data["subset_data"]), 16)]
    ...
    ax.set_yticks([i for i, _ in enumerate(range(thresholds.size)) if i % 8 == 0],
                  [f"{v:1f}" for i, v in enumerate(thresholds.tolist()) if i % 8 == 0])
    plt.show()
