...
fig, ax = plt.subplots(layout="tight")
ax.imshow(np.zeros_like(masks[0]), alpha=0)
plot_contour_spaghetti([masks[i] for i in masks_filter], memberships=filtered_memberships, highlight=outlier_filter,
                       ax=ax)
plt.show()
...
def get_relative_depths(data_matrix, clustering, medoids_idx):
    ...
    for i in range(data_matrix.shape[0]):  # relative depth per x
        ...
        between_depths.append(
            [competing_medoid, ] + compute_l_depth(data_matrix, (i,), competing_medoid_cluster_idx).tolist()[0])
...
def get_cost_function_info(data_matrix, clustering, medoids_idx):
    ...
    df = pd.DataFrame(
        [within_depths[:, 0], kmedoids_labels, within_depths[:, 1], between_depths[:, 2], between_depths[:, 0],
         reds[:, 1],
         a[:, 1], b[:, 1], sils[:, 1]]).T
