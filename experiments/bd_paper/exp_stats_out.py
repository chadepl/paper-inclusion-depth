...
df_exp["outs_idx"] = df_exp["depths"].apply(lambda v: np.argsort(v)[:get_num_outs(
    v)])  # we get the ids of the contours with the 10 contours with the smallest depth
...
for col in cols:
    ...
    df_outs[new_cols[-1]] = df_outs.apply(
        lambda x: perc_outliers_identified(x[col], x["outs_idx mtbad"], adjust_size=False), axis=1)
...
formatted_latex_table = formatted_latex_table.groupby(["reference", "method"]).agg(
