...
def compute_depths(contours, ensemble=None, modified=False, global_criteria="nestedness",
                   border_functions=None, return_point_depths=False, return_border_coords=False, times_dict=None, use_fast=False):
    ...
    if use_fast:
        ...
        global_depths, point_depths, border_coords = compute_depths_fast(contours, ensemble=ensemble, mode="inner", modified=False,
                            return_point_depths=True, return_border_coords=True, times_dict=times_dict)
    else:
        ...
        if border_functions is None:
            if global_criteria == "l2_dist":
                border_functions, ensemble_sdfs, border_coords = get_border_functions(contours, ensemble, return_sdfs=True, return_border_coords=True)
            else:
                border_functions, border_coords = get_border_functions(contours, ensemble, return_sdfs=False, return_border_coords=True)
        else:
            if global_criteria == "l2_dist":
                border_functions, ensemble_sdfs, border_coords = get_border_functions(contours, ensemble, return_sdfs=True, return_border_coords=True)
...
if __name__ == "__main__":
    ...
    from backend.src.datasets.circles import circles_different_radii_spreads, circles_multiple_radii_modes, circles_with_outliers
    from backend.src.datasets.bd_paper import get_contaminated_contour_ensemble_magnitude, get_contaminated_contour_ensemble_shape, get_contaminated_contour_ensemble_topological
