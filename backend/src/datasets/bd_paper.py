...
def get_contaminated_contour_ensemble_shape(num_members, num_rows, num_cols, num_vertices=100, p_contamination=0.2, scale=0.01, freq=0.01, return_labels=False):
    ...
    should_contaminate = (np.random.random(num_members) > (1 - p_contamination)).astype(float).reshape(num_members, -1).repeat(num_vertices, axis=1)
...
def get_contaminated_contour_ensemble_topological(num_members, num_rows, num_cols, num_vertices=100, p_contamination=0.2, return_labels=False):
    ...
    random_pos = np.random.choice(np.arange(xs.shape[1]), xs.shape[0], replace=True)  # we get a point along the boundary per curve
    ...
    angle = np.array([thetas[0, rp] for i, rp in enumerate(random_pos)]).reshape(-1, 1)#np.arctan2(random_xs, random_ys)
    ...
    xs_out = xs_out + (magnitude + topo_feat_sign * (max_xs_out + random_margin)) * np.cos(angle)# + scaled_radii_out.max(axis=1).reshape(-1, 1)*(num_cols)) * np.cos(angle)#(scaled_radii_out * num_cols//2).max(axis=1).reshape(-1, 1)
    ys_out = ys_out + (magnitude + topo_feat_sign * (max_ys_out + random_margin)) * np.sin(angle) # + scaled_radii_out.max(axis=1).reshape(-1, 1)*(num_rows)) * np.sin(angle) #(scaled_radii_out * num_rows//2).max(axis=1).reshape(-1, 1)
    xs_out = xs_out + num_cols // 2
    ys_out = ys_out + num_cols // 2
...
def get_han_dataset_ParotidR(num_rows, num_cols):
    ...
    img, gt, ensemble_masks = get_han_slice_ensemble(num_rows, num_cols, patient_id=0, structure_name="Parotid_R", slice_num=41)
    return img, gt, ensemble_masks

def get_han_dataset_BrainStem(num_rows, num_cols):
    from backend.src.datasets.han_ensembles import get_han_slice_ensemble
    img, gt, ensemble_masks = get_han_slice_ensemble(num_rows, num_cols, patient_id=0, structure_name="BrainStem", slice_num=31)
    return img, gt, ensemble_masks
...
if __name__ == "__main__":
    ...
    datasets = [
        ...
        #                                         p_contamination=0.2, return_labels=True),
        get_contaminated_contour_ensemble_topological(num_members, num_rows, num_cols, p_contamination=0.1, return_labels=True)
    ]
