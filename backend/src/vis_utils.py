...
def plot_contour(contour, ax=None, plot_line=True, line_kwargs=None, plot_markers=False, markers_kwargs=None, smooth_line=True):
    if ax is None:
        pobj = plt
    else:
        pobj = ax
...
def plot_contour_spaghetti(masks, under_mask=None, arr=None, is_arr_categorical=True, vmin=None, vmax=None,
    ...
    for i, contour in enumerate(contours):
        plot_contour(contour, line_kwargs=dict(linewidth=linewidth, color=cs[i], alpha=alpha, zorder=0), smooth_line=smooth_line, ax=ax)

    for i in highlight:
        plot_contour(contours[i], plot_line=False, plot_markers=True, markers_kwargs=dict(color="red", s=1, zorder=1), smooth_line=smooth_line, ax=ax)

    ax.set_axis_off()
    return ax


def plot_contour_boxplot(masks, depths, outlier_type="tail", epsilon_out=3, show_out=True, under_mask=None, ax=None, smooth_line=True):
    """
    ...
    if show_out:
        for outlier_id in cbp_outliers:
            ...
            plot_contour(contours, line_kwargs=dict(c="red", linestyle="dashed", linewidth=1, alpha=0.8), smooth_line=smooth_line, ax=ax)
