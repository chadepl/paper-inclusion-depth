...
sdfs_abs = [np.abs(get_distance_transform(m, tf_type="signed")) for m in masks]  # needed to extract band around iso-line
...
for i, c in enumerate(member_contours[member_id]):
    ...
    plt.scatter(member_sps[member_id][i][:, 1], member_sps[member_id][i][:, 0], s=20, c=color, linewidths=1, edgecolors="white", zorder=1)
plt.colorbar()
plt.show()
...
for cps in ref_contour_points:
    connectivity = np.concatenate([np.arange(cps.shape[0]).reshape(-1,1), np.concatenate([np.arange(cps.shape[0])[1:], np.zeros(1)]).reshape(-1,1)], axis=1)
...
for i, (cps, connects) in enumerate(zip(ref_contour_points, ref_connectivities)):
    ...
    for j in range(connects.shape[0]):
        ...
        rectangles_outer.append(Rectangle(cps[p0], width=seg_vec_len, height=lw * linewidth_factor, angle=rotation, facecolor="green"))
        rectangles_inner.append(Rectangle(cps[p0], width=seg_vec_len, height=lw * linewidth_factor, angle=rotation+180, facecolor="red"))
