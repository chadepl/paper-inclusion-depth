...
df_exp["ins_idx"] = df_exp["depths"].apply(lambda v: np.argsort(v)[get_num_outs(v):])  # we get the ids of the contours with the N - N \times alpha largest depths
...
for index, row in df_ins.iterrows():
    ...
    # Mean
    mean_arr_ref_est = np.concatenate([ensemble[i][:, :, np.newaxis] for i in row["ins_idx mtbad"]], axis=-1).mean(axis=-1).squeeze()
...
formatted_latex_table = formatted_latex_table.groupby(["statistic", "method"]).agg(lambda r: f"{r[0]:.8f} pm {r[1]:.8f}")
formatted_latex_table = formatted_latex_table.T
