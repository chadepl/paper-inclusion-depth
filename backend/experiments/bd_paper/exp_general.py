...
from backend.src.datasets.bd_paper import get_contaminated_contour_ensemble_center, get_contaminated_contour_ensemble_magnitude, get_contaminated_contour_ensemble_shape, get_contaminated_contour_ensemble_topological
...
sizes = np.concatenate([np.arange(10, size_limit_bad + 1, samples_spacing).astype(int), np.arange(size_limit_bad, max_size + 1, samples_spacing * 5).astype(int)[1:]])
datasets = dict(
    ...
    cont_topo=lambda nm,nr,nc: get_contaminated_contour_ensemble_topological(nm, nr, nc, return_labels=True, p_contamination=0.05)
                 )
methods = dict(
    ...
    mbod_nest=lambda ensemble, td: border_depth.compute_depths(ensemble, modified=True, global_criteria="nestedness", times_dict=td),
            ...
            if entries_path.exists():
                ...
                for s in sizes:
                    if not (s > size_limit_bad and "bad" in method_name):
                        ...
                        print(f"Processing: dataset: {dataset_name} - size: {s} - method: {method_name} - replication: {r}")
