...
def get_rectangles_dataset(num_cols, num_rows, num_members, variation="location"):
    ...
    rectangle_rr_cc = [[l.astype(np.int) for l in rectangle(starts[i], ends[i], shape=(num_rows, num_cols))] for i in
                       range(num_members)]
...
def get_shapes_dataset(num_cols, num_rows, num_members, fraction_squares=1 / 3, fraction_circles=1 / 3,
                       fraction_triangles=1 / 3):
