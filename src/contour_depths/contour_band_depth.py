...
def get_depth_matrix(depth_data, raw_quantity="max_lc_rc", threshold=None):
    ...
    if threshold is None:
        ...
        contains_th[
            np.logical_and(depth_df["max_lc_rc"].to_numpy() < threshold, depth_df["exclude"].to_numpy() == 0)] = 1
        contains_th[
            np.logical_and(depth_df["max_lc_rc"].to_numpy() >= threshold, depth_df["exclude"].to_numpy() == 0)] = 0
